...
\textit{计算机科学硕士 \textnormal{\small（Digital Skills for Health Transformation track）}} \hfill \textit{2025.09 -- 2027.09}
\begin{itemize}
...
上述两篇论文的（共同）一作。端到端负责实验流水线（benchmark 与 baseline 实现、训练 / 推理基础设施、消融实验、图表生产），并主导两篇投稿的 framing 与写作。
...
cfDNA 特征表示 $+$ stacked ensemble 用于结直肠癌早筛 \tgenomics；负责特征工程、模型选择与交叉验证。在 held-out 队列上达到 \textbf{94.88\%} 灵敏度。
