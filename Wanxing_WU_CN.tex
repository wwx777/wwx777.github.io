...
\textbf{巴黎理工学院（IP Paris）}计算机科学硕士在读，本科毕业于\textbf{南方科技大学}（数据科学与大数据技术），现为 \textbf{SUSTech-NLP} 科研助理（导师：陈冠华教授）。研究方向：\textbf{异构能力 LLM 协作}。在 \textbf{RouterXBench}（per-query 路由）中设计了 router 能力与场景解耦的 3 轴评估框架，发现\emph{模型内部 hidden state} 比 verbose / logit / 外部 embedding 都更适合做 routing。在 \textbf{Scope-then-Cover}（agent 适应）中利用 exploration 内部的能力非对称：强 actor 提名一个高 recall 的小子空间，冻结廉价 worker 并行覆盖，封装为单一 \texttt{explore} action。

% ── EDUCATION ────────────────────────────────────────────────
\section{教育经历}

\entry{\unilogo{ipp.png}\enskip 巴黎理工学院（Institut Polytechnique de Paris）}{法国巴黎}
\textit{计算机科学硕士} \hfill \textit{2025.09 -- 至今}
\begin{itemize}
